...
int oplus_display_set_vendor(struct dsi_display *display)
{
	...
	if (!strcmp(display->display_type, "secondary"))
		register_device_proc("lcd_s", (char *)display->panel->oplus_panel.vendor_name,
				(char *)display->panel->oplus_panel.manufacture_name);

	register_device_proc("lcd", (char *)display->panel->oplus_panel.vendor_name,
			(char *)display->panel->oplus_panel.manufacture_name);
		...
		size_t len)
{
	...
	if (rc < 0) {
		OPLUS_DSI_ERR("[%s] failed to read panel register, rc=%d,cmd=%d\n",
				display->name,
				rc,
				cmd);
	}
		...
		const char *buf, size_t count)
{
	...
	OPLUS_DSI_INFO("oplus_display_set_seed = %d\n", temp_save);
	...
	if (display->panel->power_mode == SDE_MODE_DPMS_ON) {
		...
		OPLUS_DSI_ERR("oplus_display_set_seed = %d, but now display panel status is not on\n",
				temp_save);
	}
		...
		const char *buf, size_t count)
{
	...
	if (display->panel->power_mode == SDE_MODE_DPMS_ON) {
		...
		OPLUS_DSI_ERR("oplus_display_set_spr = %d, but now display panel status is not on\n",
				temp_save);
	}
...
{
	OPLUS_DSI_INFO("oplus_display_get_seed = %d\n", seed_mode);
...
static ssize_t oplus_display_get_dither(struct kobject *obj,
		struct kobj_attribute *attr, char *buf)
{
	OPLUS_DSI_INFO("oplus_display_get_dither = %d\n", dither_enable);
	return sysfs_emit(buf, "%d\n", dither_enable);
}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	ret = oplus_display_read_serial_number(display, &serial_number);
	if (ret < 0) {
		OPLUS_DSI_ERR("Get panel serial number failed\n");
		ret = sysfs_emit(buf, "Get panel serial number failed, reason:%d\n", ret);
	} else {
		ret = sysfs_emit(buf, "Get panel serial number: %016lX", serial_number);
	}
...
int oplus_display_get_panel_btbsn_sub(struct dsi_display *display, char *btbsn_buf)
{
	...
	if (!display->panel->oplus_panel.btb_sn.btb_sn_support) {
		OPLUS_DSI_INFO("display panel btb sn not support\n");
		return -EINVAL;
	}

	/*
	 * for some unknown reason, the panel_btb_sn may read dummy,
	 * retry when found panel_btb_sn is abnormal.
	 */
	for (i = 0; i < 5; i++) {
		if (display->panel->power_mode != SDE_MODE_DPMS_ON) {
		...
		if (display->panel->oplus_panel.btb_sn.is_switch_page) {
			mutex_lock(&display->display_lock);
			mutex_lock(&display->panel->panel_lock);
			ret = dsi_panel_tx_cmd_set(display->panel, DSI_CMD_PANEL_BTB_SWITCH, false);
			mutex_unlock(&display->panel->panel_lock);
			mutex_unlock(&display->display_lock);
			if (ret) {
				OPLUS_DSI_ERR("get btb sn switch page failed\n");
				return ret;
			}
		}

		mutex_lock(&display->display_lock);
		ret = dsi_display_read_panel_reg(display, display->panel->oplus_panel.btb_sn.btb_sn_reg,
				btbsn_buf, display->panel->oplus_panel.btb_sn.btb_sn_conut);
		mutex_unlock(&display->display_lock);
		...
		if (display->panel->oplus_panel.btb_sn.is_switch_page) {
			...
			if (ret) {
				OPLUS_DSI_ERR("Failed to send DSI_CMD_DEFAULT_SWITCH_PAGE\n");
				mutex_unlock(&display->panel->panel_lock);
				mutex_unlock(&display->display_lock);
				return ret;
			}
...
static ssize_t oplus_display_sysfs_get_panel_btbsn(struct kobject *obj,
		struct kobj_attribute *attr, char *buf)
{
	int ret = 0;
	...
	if (!display || !display->panel) {
		ret = sysfs_emit(buf, "Get panel btb sn : failed for panel is null");
		OPLUS_DSI_ERR("%s\n", buf);
		return ret;
	}
	...
	if(display->enabled == false) {
		...
		display = get_sec_display();
		if (!display) {
			ret = sysfs_emit(buf, "Get panel btb sn : failed for second panel is null");
			OPLUS_DSI_WARN("%s\n", buf);
			return ret;
		}
		if (display->enabled == false) {
			ret = sysfs_emit(buf, "Get panel btb sn : failed for panel is off");
			OPLUS_DSI_WARN("%s\n", buf);
			return ret;
		}
	}
	...
	if (!oplus_display_get_panel_btbsn_sub(display, btbsn_buf)) {
		ret = sysfs_emit(buf, "Get panel btb sn: %s", btbsn_buf + display->panel->oplus_panel.btb_sn.btb_sn_index);
		OPLUS_DSI_INFO("%s\n", buf);
	} else {
		ret = sysfs_emit(buf, "Get panel btb sn: read failed");
		OPLUS_DSI_ERR("%s\n", buf);
	}
...
static ssize_t oplus_display_get_panel_apl_value(struct kobject *obj,
		struct kobj_attribute *attr, char *buf)
{
	int cnt = 0;
	struct dsi_display *display = get_main_display();
	SDE_ATRACE_BEGIN("get_panel_apl_value");
	...
	mutex_lock(&display->display_lock);
	cnt = oplus_get_panel_apl_value(buf);
	mutex_unlock(&display->display_lock);
	SDE_ATRACE_END("get_panel_apl_value");
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	if (temp_display->modes == NULL) {
		OPLUS_DSI_ERR("display mode is null\n");
		ret = -1;
		return ret;
	}
		...
		const char *buf, size_t count)
{
	...
	OPLUS_DSI_ERR("lcd_closebl_flag = %d\n", closebl);

	if (1 != closebl) {
		lcd_closebl_flag = 0;
	}

	OPLUS_DSI_ERR("oplus_display_set_closebl_flag = %d\n", lcd_closebl_flag);
	return count;
}
...
static ssize_t oplus_backlight_smooth_get_debug(struct kobject *obj,
	struct kobj_attribute *attr, char *buf)
{
	OPLUS_DSI_INFO("oplus_backlight_smooth_get_debug = %d\n", backlight_smooth_enable);
	return sysfs_emit(buf, "%d\n", backlight_smooth_enable);
}
		...
		const char *buf, size_t count)
{
	...
	OPLUS_DSI_ERR("backlight_smooth_enable = %d\n", bk_feature);
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	if (!panel->oplus_panel.ffc_enabled) {
		OPLUS_DSI_ERR("Falied to get ffc mode, because it is unsupport\n");
		rc = -EFAULT;
		return rc;
	}

	mutex_lock(&panel->oplus_panel.oplus_ffc_lock);
	ffc_mode = panel->oplus_panel.ffc_mode_index;
	mutex_unlock(&panel->oplus_panel.oplus_ffc_lock);
	OPLUS_DSI_INFO("Get ffc mode: %d\n", ffc_mode);
		...
		const char *buf, size_t count)
{
	...
	if (!panel->oplus_panel.ffc_enabled) {
		OPLUS_DSI_ERR("Falied to set ffc mode, because it is unsupport\n");
		rc = -EFAULT;
		return rc;
	}
	...
	OPLUS_DSI_INFO("Set ffc mode: %d\n", ffc_mode);

	if (ffc_mode > FFC_MODE_MAX_COUNT) {
		OPLUS_DSI_ERR("Invalid ffc mode:[%d]\n", ffc_mode);
		rc = -EINVAL;
		return rc;
	}

	mutex_lock(&panel->oplus_panel.oplus_ffc_lock);
	panel->oplus_panel.ffc_mode_index = ffc_mode;

	mutex_lock(&panel->panel_lock);
	rc = oplus_panel_set_ffc_mode_unlock(panel);
	mutex_unlock(&panel->panel_lock);

	mutex_unlock(&display->panel->oplus_panel.oplus_ffc_lock);
		...
		const char *buf, size_t count)
{
	...
	if (rc) {
		OPLUS_DSI_WARN("%s cannot be converted to u32", buf);
		return count;
	}
...
static void oplus_display_print_cmd_desc(const struct dsi_panel_cmd_set *cmd_sets)
{
	...
	for (i = 0; i < cmd_sets->count; i++) {
		...
		if (msg.tx_len >= (PANEL_TX_MAX_BUF / 2)) {
			OPLUS_DSI_ERR("%s Skip current cmds[%d], invalid msg.tx_len=%lu\n",
		...
		len += snprintf(buf + len, sizeof(buf) - len, "%02X %02X",
				(char)msg.tx_len >> 8, (char)msg.tx_len & 0xFF);
...
static int oplus_display_dump_dsi_command(struct dsi_display *display)
{
	...
	if (!display || !display->panel || !display->panel->cur_mode) {
		OPLUS_DSI_ERR("failed to get main dsi display\n");
		return -EFAULT;
	}
	...
	if (!mode || !mode->priv_info) {
		OPLUS_DSI_ERR("failed to get dsi display mode\n");
		return -EFAULT;
	}
...
static int oplus_panel_get_cmd_pkt_count(const char *data, u32 length,
		u32 *cnt)
{
	...
	while (length >= cmd_set_min_size) {
		...
		if (packet_length > length) {
			OPLUS_DSI_ERR("format error packet_length[%d] length[%d] count[%d]\n",
...
static int oplus_panel_create_cmd_packets(const char *data,
		u32 length,
		u32 count,
		...
		const char *buf, size_t count)
{
	...
	if (send) {
		...
		if (rc) {
			OPLUS_DSI_ERR("failed to send dcs[%d], rc=%d\n", oplus_dsi_command, rc);
			return rc;
		}
		return count;
	}
	...
	if (old_dsi_command >= DSI_CMD_SET_MAX) {
		OPLUS_DSI_ERR("UnSupport dsi command set\n");
		goto error;
	}
	...
	rc = oplus_panel_get_cmd_pkt_count(cmd_bufs, cmd_counts,
			&packet_count);

	if (rc) {
		OPLUS_DSI_ERR("commands failed, rc=%d\n", rc);
		goto error;
	}
	...
	rc = oplus_panel_create_cmd_packets(cmd_bufs, cmd_counts,
			packet_count, cmds);

	if (rc) {
		OPLUS_DSI_ERR("failed to create cmd packets, rc=%d\n", rc);
...
static int oplus_boe_data_dimming_process_unlock(int brightness, int enable)
{
	...
	if (!dsi_panel_initialized(panel)) {
		OPLUS_DSI_ERR("dsi panel is not init\n");
		return -EINVAL;
	}
...
int oplus_panel_process_dimming_v3_post(struct dsi_panel *panel, int brightness)
{
	...
	if (enable != dimming_enable) {
		if (enable) {
			...
			if (ret) {
				OPLUS_DSI_ERR("failed to enable data dimming\n");
				goto error;
			}

			dimming_enable = true;
			OPLUS_DSI_ERR("Enter DC backlight v3\n");
			...
			OPLUS_DSI_ERR("Exit DC backlight v3\n");
		}
	}
		...
		const char *buf, size_t count)
{
	...
	if (display && display->name) {
		...
		if (!dsi_connector || !dsi_connector->state || !dsi_connector->state->crtc) {
			OPLUS_DSI_ERR("display not ready\n");

		} else {
			err = drm_crtc_vblank_get(dsi_connector->state->crtc);

			if (err) {
				OPLUS_DSI_ERR("failed to get crtc vblank, error=%d\n", err);
		...
		const char *buf, size_t count)
{
	...
	OPLUS_DSI_ERR("debug for oplus_display_set_esd_status, the enable value = %d\n",
			enable);
...
int dsi_update_dynamic_osc_clock(void)
{
	...
	if (!display->panel->oplus_panel.is_osc_support) {
		OPLUS_DSI_ERR("not support osc\n");
		return 0;
	}
	...
	if (osc_clock_rate) {
		...
		} else if (osc_clock_rate == display->panel->oplus_panel.osc_clk_mode1_rate) {
			rc = dsi_panel_tx_cmd_set(display->panel, DSI_CMD_OSC_CLK_MODEO1, false);
		} else {
			OPLUS_DSI_ERR("unsupport osc clk rate=%d\n", osc_clock_rate);
		}
		if (rc)
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	rc = sysfs_emit(buf, "%d\n", dynamic_osc_clock);
	OPLUS_DSI_INFO("read dsi clk rate %d\n", dynamic_osc_clock);
		...
		const char *buf, size_t count)
{
	...
	if(display->panel->oplus_panel.is_osc_support) {
		...
		OPLUS_DSI_INFO("osc clk param value: '%d'\n", osc_clk);
		...
		const char *buf, size_t count)
{
	...
	OPLUS_DSI_INFO("buf = [%s], id = %d value = %d, count = %lu\n",
	...
	return sysfs_emit(buf, "Support trackpoint id:\n \
		...
		*** --> Trackpoint test default use %d\n \
		Triggered trackpoint id: %d\n",
			...
			OPLUS_DISP_Q_ERROR_FENCE_TIMEOUT,
...
static OPLUS_ATTR(panel_btb_sn, S_IRUGO | S_IWUSR,
		oplus_display_sysfs_get_panel_btbsn, NULL);
