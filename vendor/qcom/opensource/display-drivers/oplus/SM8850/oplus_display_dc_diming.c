...
**
** File : oplus_display_dc_diming.c
...
static struct dsi_panel_cmd_set oplus_panel_seed_cmd_set;
...
int oplus_panel_parse_dc_config(struct dsi_panel *panel)
{
	...
	if (!arr) {
		OPLUS_DSI_ERR("[%s] oplus,dsi-dc-brightness  not found\n", panel->name);
		return -EINVAL;
	}

	if (length & 0x1) {
		OPLUS_DSI_ERR("[%s] oplus,dsi-dc-brightness length error\n", panel->name);
		return -EINVAL;
	}

	OPLUS_DSI_DEBUG("RESET SEQ LENGTH = %d\n", length);
	...
	rc = utils->read_u32_array(utils->data, "oplus,dsi-dc-brightness",
					arr_32, length);
	if (rc) {
		OPLUS_DSI_ERR("[%s] cannot read dsi-dc-brightness\n", panel->name);
	...
	panel->oplus_panel.dc_ba_seq = seq;
	panel->oplus_panel.dc_ba_count = count;
...
int sde_connector_update_backlight(struct drm_connector *connector, bool post)
{
	...
	if (!dsi_display || !dsi_display->panel || !dsi_display->panel->cur_mode) {
		OPLUS_DSI_ERR("Invalid params(s) dsi_display %pK, panel %pK\n",
			  dsi_display,
...
int oplus_seed_bright_to_alpha(int brightness)
{
	...
	if (display->panel->oplus_panel.dc_ba_seq && display->panel->oplus_panel.dc_ba_count) {
		count = display->panel->oplus_panel.dc_ba_count;
		lut = display->panel->oplus_panel.dc_ba_seq;
	} else {
		...
		enum dsi_cmd_set_type type)
{
	...
	if (oplus_panel_seed_cmd_set.cmds) {
		for (i = 0; i < oplus_panel_seed_cmd_set.count; i++) {
			kfree(oplus_panel_seed_cmd_set.cmds[i].msg.tx_buf);
		}

		kfree(oplus_panel_seed_cmd_set.cmds);
	}

	oplus_panel_seed_cmd_set.cmds = oplus_cmd;
	oplus_panel_seed_cmd_set.count = count;
	oplus_panel_seed_cmd_set.state = state;
	oplus_dc2_alpha = alpha;

	return &oplus_panel_seed_cmd_set;
