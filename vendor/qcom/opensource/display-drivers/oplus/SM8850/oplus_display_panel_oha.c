...
void oplus_panel_oha_init(struct dsi_panel *panel)
{
	...
	if (inited && oplus_oha_config) {
		OPLUS_DSI_WARN("kOFP oha config = %#X already\n", oplus_oha_config);
		return;
	}
	...
	OPLUS_DSI_INFO("kOFP oha config = %#X\n", oplus_oha_config);
}
		...
		struct dsi_panel *panel)
{
	...
	if (!cstate) {
		OPLUS_DSI_ERR("kOFP, Invalid params cstate null\n");
		return 0;
	}
	...
	if (!count) {
		OPLUS_DSI_ERR("This panel does not support oplus oha mode\n");
		goto error;
	}
		...
		struct dsi_display *display)
{
	...
	if (!display || !display->panel) {
		OPLUS_DSI_ERR("kOFP, Invalid params(s) display %pK, panel %pK\n",
...
/*
	if (display->panel->is_hbm_enabled) {
		OPLUS_DSI_ERR("error panel->is_hbm_enabled\n");
		return -EINVAL;
	}
*/

	if (get_oplus_display_scene() != OPLUS_DISPLAY_AOD_SCENE) {
		OPLUS_DSI_ERR("get_oplus_display_scene = %d, return\n",
	...
	if (!dsi_panel_initialized(display->panel)) {
		OPLUS_DSI_ERR("oplus_display_update_oha_mode is not init\n");
		rc = -EINVAL;
		goto error;
	}
	...
	if (rc) {
		OPLUS_DSI_ERR("[%s] failed to oplus_panel_update_oha_mode_unlock, rc=%d\n",
				display->name, rc);
		goto error;
	}
...
static void oplus_oha_update_init(void)
{
	if (!oha_update_task) {
		...
		OPLUS_DSI_INFO("[oha_update] init CREATE\n");
	}
}
...
void oplus_oha_update(void)
{
	if (oha_update_task != NULL) {
		...
		OPLUS_DSI_INFO("[oha_update] update is NULL\n");
	}
}*/
		...
		struct dsi_display *dsi_display)
{
	...
	if (!dsi_display || !dsi_display->panel) {
		OPLUS_DSI_ERR("kOFP, Invalid params(s) dsi_display %pK, panel %pK\n",
			  dsi_display,
	...
	if (!dsi_display->panel->panel_initialized) {
		...
		OPLUS_DSI_ERR("kOFP, panel not initialized, failed to Enter Oha\n");
		return 0;
	}
	...
	if (rc) {
		OPLUS_DSI_ERR("kOFP, oplus_display_update_oha_mode failed, rc=%d\n", rc);
		return rc;
	}
		...
		struct dsi_display *dsi_display)
{
	...
	if (!dsi_display->panel->panel_initialized) {
		...
		OPLUS_DSI_ERR("kOFP, panel not initialized, failed to Exit Oha\n");
		return 0;
	}
...
int oplus_connector_ofp_update_oha(struct drm_connector *connector)
{
	...
	if (!c_conn) {
		OPLUS_DSI_ERR("kOFP, Invalid params oha sde_connector null\n");
		return -EINVAL;
	}
	...
	if (!dsi_display || !dsi_display->panel) {
		OPLUS_DSI_ERR("kOFP, Invalid params(s) oha dsi_display %pK, panel %pK\n",
			  dsi_display,
			  ((dsi_display) ? dsi_display->panel : NULL));
		return -EINVAL;
	}
	...
	if (oha_mode != dsi_display->panel->is_oha_enabled) {
		if (OPLUS_DISPLAY_AOD_SCENE == get_oplus_display_scene()) {
			...
			if (oha_mode) {
				...
				if (rc) {
					OPLUS_DSI_ERR("kOFP, failed to oplus_ofp_update_oha_enter, rc=%d\n", rc);
				}
			} else {
				rc = oplus_ofp_update_oha_exit(c_conn->encoder->crtc->state, dsi_display);
				if (rc) {
					OPLUS_DSI_ERR("kOFP, failed to oplus_ofp_update_oha_exit, rc=%d\n", rc);
				}
			}
...
int oplus_display_panel_set_oha_enable(void *data)
{
	...
	OPLUS_DSI_ERR("kOFP, systemui set oha_enable = %d\n", value);
