...
static struct iris_memc_helper_ops memc_helper_ops;

struct pw_iris_dsc_info {
...
static enum iris_chip_type iris_chip_type = CHIP_UNKNOWN;

static void _iris_get_chip_type(void)
{
	iris_chip_type = iris_get_chip_type();
}
...
static void _iris_dsc_parse_payload(uint32_t *payload, uint32_t pps_sel)
{
	...
	uint32_t chroma_format = IRIS_CHROMA_422;
	...
	if (chroma_format != IRIS_CHROMA_444 && chroma_format != IRIS_CHROMA_422)
		chroma_format = IRIS_CHROMA_444;
	...
	if (iris_dsc_info[pps_sel].version_minor == 0x01 &&
			iris_dsc_info[pps_sel].chroma_format != IRIS_CHROMA_444)
		iris_dsc_info[pps_sel].chroma_format = IRIS_CHROMA_444;
}
...
static void _iris_dsc_parse_info(void)
{
	...
	IRIS_LOGI("%s(), MEMC DSC1.%u, fmt: %s, pc: 0x%08x, sl: %u, 0x%08x, bpc: %u, bpp: %u",
			__func__,
			iris_dsc_info[DSC_PPS_SET0].version_minor,
			iris_dsc_info[DSC_PPS_SET0].chroma_format == IRIS_CHROMA_444 ? "444" : "422",
	...
	IRIS_LOGI("%s(), RFB DSC1.%u, fmt: %s, pc: 0x%08x, sl: %u, 0x%08x, bpc: %u, bpp: %u",
			__func__,
			iris_dsc_info[DSC_PPS_SET1].version_minor,
			iris_dsc_info[DSC_PPS_SET1].chroma_format == IRIS_CHROMA_444 ? "444" : "422",
...
static uint8_t _iris_dsc_dec_initial_tc(uint32_t pps_sel)
{
	...
	if (iris_dsc_info[pps_sel].chroma_format == IRIS_CHROMA_422)
		fac = 2;
...
static void _iris_dsc_update_pwil(uint32_t pps_sel, bool send)
{
	...
	if (pps_sel == DSC_PPS_SET1)
		payload[0] = BITS_SET(payload[0], 1, 22, rfb_bitwidth);  // PSR_BITWIDTH

	/* Sync for fps switch with different clock */
	iris_force_sync_payload(IRIS_IP_PWIL, 0x01, 4, payload[0]);
	iris_force_sync_payload(IRIS_IP_PWIL, 0x01, 5, payload[1]);
...
static void _iris_dsc_up_format(bool up, uint32_t *payload, uint32_t pps_sel)
{
	switch (iris_chip_type) {
	case CHIP_IRIS7:
	case CHIP_IRIS7P:
		break;
	case CHIP_IRIS8:
		if (memc_helper_ops.iris_dsc_up_format)
			memc_helper_ops.iris_dsc_up_format(up, payload, pps_sel);
		break;
	default:
		IRIS_LOGE("%s(), doesn't support for chip type: %#x",
				__func__, iris_chip_type);
	}
}
...
static void _iris_dsc_update_ctrl(uint32_t pps_sel)
{
	...
	_iris_dsc_up_format(iris_dsc_info[pps_sel].chroma_format == IRIS_CHROMA_422,
					payload, pps_sel);
...
static void _iris_dsc_update_pps(uint32_t pps_sel, bool trig)
{
	uint8_t chain = trig ? 1 : 0;

	if (iris_chip_type == CHIP_IRIS8 && pps_sel == DSC_PPS_SET0)
		chain = 1;

	iris_init_update_ipopt_t(IRIS_IP_DSC_DEN_2, 0x01, 0x01, 1);
	iris_init_update_ipopt_t(IRIS_IP_DSC_DEN_3, 0x01, 0x01, 1);
	iris_init_update_ipopt_t(IRIS_IP_DSC_ENC_2, 0x00, 0x00, chain);
}

static void _iris_dsc_sync_enc(uint8_t ip_des, uint8_t opt_des,
		uint8_t ip_src, uint8_t opt_src)
{
	struct iris_ip_opt *des_popt = NULL;
	struct iris_ip_opt *src_popt = NULL;
	const int32_t offset = 2 * sizeof(int32_t);
	int32_t i = 0;

	des_popt = iris_find_ip_opt(ip_des, opt_des);
	src_popt = iris_find_ip_opt(ip_src, opt_src);
	if (des_popt == NULL || src_popt == NULL) {
		IRIS_LOGW_IF(des_popt == NULL,
				"%s(), failed to find: %02x, %02x", __func__, ip_des, opt_des);
		IRIS_LOGW_IF(src_popt == NULL,
				"%s(), failed to find: %02x, %02x", __func__, ip_src, opt_src);
		return;
	}

	IRIS_LOGI("%s(), [%02x, %02x]->[%02x, %02x], cmd count: %u", __func__,
			ip_src, opt_src, ip_des, opt_des, src_popt->cmd_cnt);

	for (i = 0; i < src_popt->cmd_cnt; i++) {
		uint8_t *pdes = (uint8_t *)des_popt->cmd[i].msg.tx_buf;
		uint8_t *psrc = (uint8_t *)src_popt->cmd[i].msg.tx_buf;

		IRIS_LOGD("%s(), offset: %u, cmd buf len: %zu", __func__,
				offset, src_popt->cmd[i].msg.tx_len);

		pdes += offset;
		psrc += offset;
		memcpy((void *)pdes, (void *)psrc,
				src_popt->cmd[i].msg.tx_len - offset);

		if (IRIS_IF_LOGV()) {
			print_hex_dump(KERN_ERR, "", DUMP_PREFIX_NONE, 32, 4,
					src_popt->cmd[i].msg.tx_buf,
					src_popt->cmd[i].msg.tx_len, false);
			print_hex_dump(KERN_ERR, "", DUMP_PREFIX_NONE, 32, 4,
					des_popt->cmd[i].msg.tx_buf,
					des_popt->cmd[i].msg.tx_len, false);
		}
	}
}

static void _iris_dsc_update_taa_enc(uint32_t pps_sel, bool trig)
{
	uint8_t chain = trig ? 1 : 0;

	if (iris_chip_type != CHIP_IRIS8)
		return;

	if (pps_sel != DSC_PPS_SET0)
		return;

	/* use TNR to replace TAA */
	IRIS_ATRACE_BEGIN(__func__);
	_iris_dsc_sync_enc(IRIS_IP_DSC_ENC_TNR, 0x00, IRIS_IP_DSC_ENC_2, 0x00);
	_iris_dsc_sync_enc(IRIS_IP_DSC_ENC_TNR, 0x01, IRIS_IP_DSC_ENC_2, 0x01);

	iris_init_update_ipopt_t(IRIS_IP_DSC_ENC_TNR, 0x00, 0x00, 1);
	iris_init_update_ipopt_t(IRIS_IP_DSC_ENC_TNR, 0x01, 0x01, chain);
	IRIS_ATRACE_END(__func__);
}

static void _iris_dsc_update_param(uint32_t pps_sel, bool trig)
{
	IRIS_ATRACE_BEGIN(__func__);

	_iris_dsc_update_ctrl(pps_sel);
	_iris_dsc_update_pps(pps_sel, trig);

	/* TAA DSC ENC follows MEMC DSC ENC strictly, change if needed. */
	_iris_dsc_update_taa_enc(pps_sel, trig);
...
static void _iris_dsc_populate_private_params(
		struct pw_iris_dsc_info *dsc_info, int intf_width)
{
	int mod_offset;
...
static void _iris_dsc_create_pps_buf_cmd(struct pw_iris_dsc_info *dsc_info,
		...
		uint32_t pps_sel)
{
	...
	if (dsc.chroma_format == IRIS_CHROMA_422) {
		dsc.config.native_422 = true;
		dsc.config.simple_422 = true;
	} else if (dsc.chroma_format == IRIS_CHROMA_420) {
		dsc.config.native_420 = true;
	}

	dsc.source_color_space = IRIS_YUV; //MSM_RGB;
...
static bool _iris_dsc_need_change(uint32_t pps_sel)
{
	...
	if (cur_info->pic_width == last_info->pic_width &&
			...
			cur_info->out_width == last_info->out_width &&
			cur_info->out_height == last_info->out_height &&
...
static void _iris_dsc_change(uint32_t pps_sel)
{
	...
	IRIS_LOGI("%s(), pc: 0x%08x, sl: %u, 0x%08x, bpc: %d, bpp: %d, ver: %d, fmt: %d, hw slice: %s",
			__func__,
	...
	trig = iris_is_pmu_frc_on();
	_iris_dsc_update_param(pps_sel, trig);
	if (trig)
...
static void _iris_memc_dsc_change(void)
{
	...
	iris_dsc_info[pps_sel].out_width = pcfg->frc_setting.disp_hres;
	iris_dsc_info[pps_sel].out_height = pcfg->frc_setting.disp_vres;
	iris_dsc_info[pps_sel].slice_width =
		iris_dsc_info[pps_sel].slice_per_pkt == 0 ?
		iris_dsc_info[pps_sel].pic_width :
		iris_dsc_info[pps_sel].pic_width / iris_dsc_info[pps_sel].slice_per_pkt;
	if (iris_dsc_info[pps_sel].pic_height % iris_dsc_info[pps_sel].slice_height)
		iris_dsc_info[pps_sel].slice_height = iris_dsc_info[pps_sel].pic_height;
...
static void _iris_memc_dsc_mv_config_specific(uint32_t count, uint32_t *values)
{
	...
	case MEMC_DSC_VER:
		...
		case 0x02: /* DSC1.2 uses YUV422 by default */
			iris_dsc_info[pps_sel].chroma_format = IRIS_CHROMA_422;
			break;
		default:
...
static void _iris_memc_dsc_dynamic_switch_pps(uint32_t count, uint32_t *values)
{
	...
	if (count > 8 &&
			(values[8] >= IRIS_CHROMA_444 && values[8] <= IRIS_CHROMA_420))
...
static bool iris_lowpower_memc;
static uint32_t iris_sr2d_prime;


static void _iris_get_expected_size(int32_t *aux_h, int32_t *aux_v)
{
	switch (iris_chip_type) {
	case CHIP_IRIS7:
	case CHIP_IRIS7P:
...
static void _iris_scl_parse_info(void)
{
	...
	scl_conf->ioinc_sel_mode = PATH_SEL_META;

	scl_conf->sr_enable = false;
	scl_conf->sr_in_h = 0;
	scl_conf->sr_in_v = 0;
	scl_conf->sr_out_h = pcfg->frc_setting.disp_hres;
	scl_conf->sr_out_v = pcfg->frc_setting.disp_vres;
	scl_conf->sr_strategy = SCL_2D_ONLY;
	scl_conf->sr_sel_mode = PATH_SEL_META;
...
static void _iris_scl_reset_param(void)
{
	...
	_iris_scl_conf[SCL_DATA_PATH0].ioinc_sel_mode = PATH_SEL_META;
	...
	_iris_scl_conf[SCL_DATA_PATH0].sr_out_h = pcfg->frc_setting.disp_hres;
	_iris_scl_conf[SCL_DATA_PATH0].sr_out_v = pcfg->frc_setting.disp_vres;
	_iris_scl_conf[SCL_DATA_PATH0].sr_sel_mode = PATH_SEL_META;
	...
	_iris_scl_conf[SCL_DATA_PATH1].ioinc_sel_mode = PATH_SEL_META;
	...
	_iris_scl_conf[SCL_DATA_PATH1].sr_out_h = pcfg->frc_setting.disp_hres;
	...
	if (payload != NULL)
		payload[0] = BITS_SET(payload[0], 2, 3, 0x00);

	payload = iris_get_ipopt_payload_data(IRIS_IP_SR, 0x01, 2);
	if (payload != NULL)
		payload[0] = BITS_SET(payload[0], 2, 22, 0x01);
}

static void _iris_ioinc_pp_init(void)
{
	iris_aux_h = 0;
	iris_aux_v = 0;
	...
	case CHIP_IRIS7P:
		break;
	case CHIP_IRIS8:
		if (memc_helper_ops.iris_ioinc_pp_init)
			memc_helper_ops.iris_ioinc_pp_init();
		break;
	default:
		IRIS_LOGE("%s(), doesn't support for chip type: %#x",
				__func__, iris_chip_type);
...
static uint32_t _iris_ioinc_change_type(bool enable,
		...
		uint32_t tap, uint32_t path_sel, uint32_t sel_mode)
{
	...
	IRIS_LOGI("%s(), path%u, current: [0x%08x->0x%08x], %u, %s, set: [0x%08x->0x%08x], %u, %s, %u",
			__func__, path_sel,
	...
	cur_conf->ioinc_sel_mode = sel_mode;

	return SCL_FULL_CHANGE;
}

static uint32_t _iris_sr_change_type(bool enable, int32_t in_h, int32_t in_v,
		int32_t out_h, int32_t out_v, uint32_t strategy,
		uint32_t path_sel, uint32_t sel_mode)
{
	struct iris_scl_conf *cur_conf = &_iris_scl_conf[path_sel];
	...
	IRIS_LOGI("%s(), path%u, current: 0x%08x->0x%08x, %u, %s, set: 0x%08x->0x%08x, %u, %s, %u",
			...
			PROC_INFO(cur_conf->sr_out_h, cur_conf->sr_out_v),
			cur_conf->sr_strategy, cur_conf->sr_enable ? "true" : "false",
			PROC_INFO(in_h, in_v), PROC_INFO(out_h, out_v),
			strategy, enable ? "true" : "false", sel_mode);

	if (cur_conf->sr_in_h == in_h && cur_conf->sr_in_v == in_v &&
			cur_conf->sr_out_h == out_h && cur_conf->sr_out_v == out_v &&
			cur_conf->sr_strategy == strategy && cur_conf->sr_sel_mode == sel_mode) {
	...
	cur_conf->sr_out_h = out_h;
	cur_conf->sr_out_v = out_v;
	cur_conf->sr_strategy = strategy;
	cur_conf->sr_sel_mode = sel_mode;
		...
		uint32_t path_sel)
{
	...
	case SCL_DATA_PATH1: /* path1 */
		...
		if (!enable) { /* restore graphic path to display res */
			struct iris_cfg *pcfg = iris_get_cfg();
	...
	default:
		IRIS_LOGE("%s(), invalid path: %u", __func__, path_sel);
	}
}

int32_t iris_calc_inc(int32_t in_hv, int32_t prec, int32_t out_hv)
{
	/* return (in_hv << prec) / out_hv; */
	return ((in_hv << prec) + out_hv / 2) / out_hv;
}

int32_t iris_calc_init_phase(int32_t hv_inc, int32_t frac, int32_t prec,
	uint32_t tap_num)
{
	/* return (hv_inc >> (frac + 1)) & */
	/* (tap_num == IOINC_TAG5 ? BITS_MSK(6) : BITS_MSK(5)); */
	int32_t pos = hv_inc >> 1;

	return ((pos - ((pos >> prec) << prec)) >> frac) &
		(tap_num == IOINC_TAG5 ? BITS_MSK(6) : BITS_MSK(5));
}

int32_t iris_calc_left_top_offset(int32_t hv_inc, int32_t prec,
	uint32_t tap_num)
{
	return (hv_inc >> (prec + 1)) + (tap_num + 1) / 2;
}

int32_t iris_calc_right_bot_offset(int32_t in_hv, int32_t out_hv,
		...
		int32_t init_phas, int32_t total_phas)
{
	...
	return out_hv - 1 - (int32_t)((delt_acc + inc - 1) / inc);
}

static void _iris_ioinc_config(int32_t in_h, int32_t in_v,
		int32_t out_h, int32_t out_v, uint32_t tap_num,
		uint32_t path_sel, uint32_t sel_mode)
{
	...
	uint32_t *payload = NULL;
	int32_t v_inc = iris_calc_inc(in_v, prec, out_v);
	int32_t v_init_phas = iris_calc_init_phase(v_inc, frac, prec, tap_num);
	int32_t v_top_offs = iris_calc_left_top_offset(v_inc, prec, tap_num);
	int32_t v_bot_offs = iris_calc_right_bot_offset(in_v, out_v, v_top_offs,
		v_inc, frac, v_init_phas, total_phas);
	int32_t v_max_lines = out_v;

	int32_t h_inc = iris_calc_inc(in_h, prec, out_h);
	int32_t h_init_phas = iris_calc_init_phase(h_inc, frac, prec, tap_num);
	int32_t h_left_offs = iris_calc_left_top_offset(h_inc, prec, tap_num);
	int32_t h_right_offs = iris_calc_right_bot_offset(in_h, out_h, h_left_offs,
		h_inc, frac, h_init_phas, total_phas);
	int32_t h_max_lines = out_h;
	uint8_t tap_sel = tap_num == IOINC_TAG5 ? 0 : 1;

	uint8_t ioinc_path_ctrl = 0x00;

	IRIS_LOGI("%s(), 0x%08x->0x%08x, tap: %d, path%u", __func__,
	...
	case SCL_DATA_PATH0: /* path0 */
		...
		IRIS_LOGI("%s(), path0[inc, alg, max, offs], v[0x%08x, 0x%08x, 0x%08x, 0x%08x], h[0x%08x, 0x%08x, 0x%08x, 0x%08x]",
			__func__,
	...
	case SCL_DATA_PATH1: /* path1 */
		...
		IRIS_LOGI("%s(), path1[inc, alg, max, offs], v[0x%08x, 0x%08x, 0x%08x, 0x%08x], h[0x%08x, 0x%08x, 0x%08x, 0x%08x]",
			__func__,
	...
	if (sel_mode == PATH_SEL_REG) {
		ioinc_path_ctrl = BIT_SET(ioinc_path_ctrl, 0);
		ioinc_path_ctrl = BITS_SET(ioinc_path_ctrl, 1, 1, path_sel & 0x1);
		/* Set Graphic proc size for path0 */
		_iris_ioinc_config_pwil(true, out_h, out_v, SCL_DATA_PATH1);
	}
	if (BITS_GET(payload[0], 2, 3) != ioinc_path_ctrl)
		payload[0] = BITS_SET(payload[0], 2, 3, ioinc_path_ctrl);
...
static bool _iris_ioinc_perform_config(bool enable, int32_t in_h, int32_t in_v,
		int32_t out_h, int32_t out_v, uint32_t path_sel, uint32_t sel_mode)
{
	...
	_iris_ioinc_config(in_h, in_v, out_h, out_v, tap_num, path_sel, sel_mode);
	_iris_ioinc_config_pwil(enable, out_h, out_v, path_sel);
...
static bool _iris_memc_ioinc_change(void)
{
	...
	IRIS_LOGI_IF(ioinc_changed, "%s(), MEMC mode: %d, [0x%08x]->[0x%08x]", __func__,
...
static void _iris_srcnn_ctrl(bool enable, uint32_t strategy,
		uint32_t path_sel, uint32_t sel_mode)
{
	uint32_t *payload = NULL;
	...
	case SCL_2D_ONLY:
		...
		if (iris_sr2d_prime & BIT_MSK(0)) {
			path_ctrl = BIT_SET(path_ctrl, 3); // GF_BYPASS
			path_ctrl = BIT_SET(path_ctrl, 5); // DE_BYPASS
			path_ctrl = BIT_SET(path_ctrl, 6); // EHN_BYPASS

			IRIS_LOGI("%s(), 2D prime.", __func__);
		}
		break;
	case SCL_CNN_ONLY:
	...
	case SCL_CNN_2D:
		path_ctrl = BIT_SET(path_ctrl, 7); // DETAIL_FORCE
		if (iris_sr2d_prime & BIT_MSK(1)) {
			path_ctrl = BIT_SET(path_ctrl, 3); // GF_BYPASS
			path_ctrl = BIT_SET(path_ctrl, 5); // DE_BYPASS
			path_ctrl = BIT_SET(path_ctrl, 6); // EHN_BYPASS

			IRIS_LOGI("%s(), CNN with 2D prime.", __func__);
		}
		break;
	default:
		break;
	...
	if (sel_mode == PATH_SEL_REG) {
		srcnn_path_ctrl = BIT_CLR(srcnn_path_ctrl, 0);
		srcnn_path_ctrl = BITS_SET(srcnn_path_ctrl, 1, 1, path_sel & 0x1);
	}
	if (BITS_GET(payload[0], 2, 22) != srcnn_path_ctrl)
		payload[0] = BITS_SET(payload[0], 2, 22, srcnn_path_ctrl);

	IRIS_LOGI("%s(), strategy: %s(%u), path%u, control: 0x%08x", __func__,
		...
		bool enable_cnn, uint32_t path_sel)
{
	...
	IRIS_LOGI("%s(), 0x%08x->0x%08x, path%u, y_h[0x%08x, 0x%08x], y_v[0x%08x, 0x%08x], uv_h[0x%08x, 0x%08x], uv_v[0x%08x, 0x%08x]",
			__func__,
		...
		int32_t out_h, int32_t out_v, uint32_t strategy)
{
	...
	if (proc_h == 0 || proc_v == 0 || out_h == 0 || out_v == 0) {
		IRIS_LOGE("%s(), invalid param: 0x%08x->0x%08x", __func__,
				PROC_INFO(proc_h, proc_v), PROC_INFO(out_h, out_v));
		return false;
	}

	ratio_h = out_h * 100 / proc_h;
	ratio_v = out_v * 100 / proc_v;
		...
		uint32_t path_sel, uint32_t sel_mode)
{
	...
	_iris_srcnn_ctrl(enable, strategy, path_sel, sel_mode);
	_iris_srcnn_calc(proc_h, proc_v, out_h, out_v, enable_cnn, path_sel);
...
static void _iris_srcnn_enable(bool enable, uint32_t path_sel)
{
	...
	IRIS_LOGI("%s(), path%u, %s", __func__,
			path_sel, enable ? "on" : "off");
...
static bool _iris_srcnn_perform_config(bool enable,
		int32_t proc_h, int32_t proc_v, int32_t out_h, int32_t out_v,
		uint32_t strategy, uint32_t path_sel, uint32_t sel_mode)
{
	uint32_t change_type = SCL_NO_CHANGE;
	...
	change_type = _iris_sr_change_type(enable, proc_h, proc_v, out_h, out_v,
			strategy, path_sel, sel_mode);
	...
	_iris_srcnn_proc(enable, proc_h, proc_v, out_h, out_v, strategy,
			path_sel, sel_mode);
...
static bool _iris_memc_srcnn_change(int32_t out_h, int32_t out_v)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	bool changed = false;
	...
	changed = _iris_srcnn_perform_config(true,
			pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres,
			out_h, out_v, iris_expected_strategy[SCL_DATA_PATH0],
			SCL_DATA_PATH0, false);

	if (changed)
		iris_init_update_ipopt_t(IRIS_IP_PWIL, 0x01, 0x01, 1);

	IRIS_LOGI_IF(changed, "%s(), sr: 0x%08x, strategy: %u", __func__,
			PROC_INFO(pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres),
...
static bool _iris_scl_lp_memc(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	bool changed = false;
	bool blend_changed = false;
	uint32_t ioinc_pos = SCL_PP_CURSOR;
	ktime_t ktime = ktime_get();

	if (iris_chip_type != CHIP_IRIS8)
		return false;

	if (!iris_lowpower_memc)
		return false;

	IRIS_ATRACE_BEGIN(__func__);

	if (iris_bld_scl_pos < SCL_PP_POS_CNT)
		ioinc_pos = iris_bld_scl_pos;

	IRIS_LOGI("%s(), blending ioinc pos: %u", __func__, ioinc_pos);

	changed = _iris_memc_ioinc_change();
	if (memc_helper_ops.iris_ioinc_pp_proc)
		changed |= memc_helper_ops.iris_ioinc_pp_proc(true, ioinc_pos,
			pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres,
			pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres,
			SCL_DATA_PATH0, PATH_SEL_META, &blend_changed);
	_iris_srcnn_config_pwil(SRCNN_MODE_DISABLE);

	if (changed) {
		uint32_t event_channels = DMA_EVENT_CH8 | DMA_EVENT_CH9;

		if (blend_changed)
			event_channels |= DMA_EVENT_CH10;
		_iris_scl_triger_dma(event_channels);
	}
...
/* For Game ECO MEMC + HESR, iMV with triple buffer case. */
static void _iris_memc_format_strategy(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();

	if (iris_chip_type != CHIP_IRIS8 && iris_chip_type != CHIP_IRIS7P)
		return;

	if (pcfg->frc_setting.vid_buf_num != 3)
		return;

	if (iris_expected_strategy[SCL_DATA_PATH0] != SCL_CNN_ONLY)
		return;

	if (pcfg->frc_setting.mv_hres * 2 < pcfg->frc_setting.disp_hres ||
		pcfg->frc_setting.mv_vres * 2 < pcfg->frc_setting.disp_vres) {
		IRIS_LOGI("%s(), format HESR strategy, [0x%08x] [0x%08x].", __func__,
			PROC_INFO(pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres),
			PROC_INFO(pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres));
		iris_expected_strategy[SCL_DATA_PATH0] = SCL_CNN_2D;
	}
}
...
static void _iris_memc_scl_change(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	bool changed = false;
	int32_t out_h = pcfg->frc_setting.disp_hres;
	int32_t out_v = pcfg->frc_setting.disp_vres;
	ktime_t ktime = ktime_get();

	IRIS_ATRACE_BEGIN(__func__);

	if (_iris_scl_lp_memc()) {
		IRIS_ATRACE_END(__func__);
		return;
	}

	_iris_get_expected_size(&out_h, &out_v);
	_iris_memc_format_strategy();

retry:
	if (!_iris_srcnn_check_valid(
				pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres,
				out_h, out_v, iris_expected_strategy[SCL_DATA_PATH0])) {
		IRIS_LOGE("%s(), invalid input: 0x%08x->0x%08x, strategy: %u, path%u",
				__func__,
				PROC_INFO(pcfg->frc_setting.mv_hres, pcfg->frc_setting.mv_vres),
				PROC_INFO(out_h, out_v),
	...
	changed = _iris_memc_ioinc_change();
	changed |= _iris_memc_srcnn_change(out_h, out_v);
...
static void _iris_scl_set_lp_memc(uint32_t mode)
{
	IRIS_LOGI("%s(), %s low power memc.", __func__,
			mode == 0 ? "disable" : "enable");

	iris_lowpower_memc = mode > 0;
}

static void _iris_scl_pp_ioinc_pos(uint32_t ioinc_pos)
{
	IRIS_LOGI("%s(), set pp ioinc pos: 0x%02x.", __func__, ioinc_pos);

	if (ioinc_pos >= 0x10)
		iris_bld_scl_pos = ioinc_pos - 0x10;
	else
		iris_bld_scl_pos = SCL_PP_POS_CNT;
}

static void _iris_scl_2d_prime(uint32_t mode)
{
	IRIS_LOGI("%s(), set 2D prime: %s(%u)", __func__,
			mode == 0 ? "off" : "on", mode);

	iris_sr2d_prime = (mode & 0x03);
}
...
static void _iris_scl_set_specific(uint32_t count, uint32_t *values)
{
	...
	case IOINC_TAP:
		...
		break;
	case LP_MEMC:
		_iris_scl_set_lp_memc(BITS_GET(values[0], 16, 0));
		break;
	case BLD_SCL_POS:
		_iris_scl_pp_ioinc_pos(BITS_GET(values[0], 16, 0));
		break;
	case SR2D_PRIME:
		_iris_scl_2d_prime(BITS_GET(values[0], 16, 0));
		break;
	default:
		break;
	}
...
static void _iris_ioinc_pp_disable(void)
{
	switch (iris_chip_type) {
	case CHIP_IRIS7:
	case CHIP_IRIS7P:
		break;
	case CHIP_IRIS8:
		if (memc_helper_ops.iris_ioinc_pp_disable)
			memc_helper_ops.iris_ioinc_pp_disable();
		break;
	default:
		IRIS_LOGE("%s(), doesn't support for chip type: %#x",
				__func__, iris_chip_type);
		...
		uint32_t scl_type)
{
	...
	case IRIS_SCL_INOUT:
		...
		break;
	case IRIS_SCL_PP:
		_iris_ioinc_pp_disable();
		IRIS_LOGI("%s(), disable PP IOINC1D.", __func__);
		break;
	case IRIS_SCL_INOUT_PP:
		_iris_ioinc_pp_disable();
		_iris_srcnn_disable();
		_iris_ioinc_disable(path_sel);
		_iris_srcnn_config_pwil_csc(false);
		_iris_scl_reset_param();
		IRIS_LOGI("%s(), disable IOINC1D, SRCNN and PP IOINC1D.", __func__);
		break;
	default:
		break;
	}
		...
		uint32_t strategy, uint32_t path_sel, uint32_t sel_mode)
{
	...
	IRIS_LOGI("%s(), scl type: %u, proc: 0x%08x, strategy: %u, path%u",
	...
	case IRIS_SCL_IN:
		changed = _iris_ioinc_perform_config(enable,
				pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres,
				proc_h, proc_v, path_sel, sel_mode);
		if (changed)
			_iris_srcnn_disable();
		break;
	case IRIS_SCL_OUT:
		if (!_iris_srcnn_check_valid(proc_h, proc_v, out_h, out_v, strategy)) {
			IRIS_LOGE("%s(), invalid input: 0x%08x, %u, for path%u", __func__,
					PROC_INFO(proc_h, proc_v), strategy, path_sel);
			return false;
		}
		changed = _iris_srcnn_perform_config(enable, proc_h, proc_v,
				out_h, out_v, strategy, path_sel, sel_mode);
		break;
	case IRIS_SCL_INOUT:
	case IRIS_SCL_INOUT_PP:
		if (!_iris_srcnn_check_valid(proc_h, proc_v, out_h, out_v, strategy)) {
		...
		uint32_t scl_type)
{
	...
	uint32_t path_sel = SCL_DATA_PATH1;
	uint32_t sel_mode = PATH_SEL_META;
	bool changed = false;

	if (count < 4 || count > 7)
		return;

	if (scl_type == IRIS_SCL_PP || scl_type == IRIS_SCL_INOUT_PP)
		return;
...
static void _iris_scl_pp_proc(uint32_t count, uint32_t *values,
		uint32_t scl_type)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	uint32_t path_sel = SCL_DATA_PATH1;
	...
	if (memc_helper_ops.iris_ioinc_pp_proc)
		changed = memc_helper_ops.iris_ioinc_pp_proc(true, values[3], values[1], values[2],
			pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres,
			path_sel, sel_mode, &blend_changed);
...
static void _iris_scl_with_pp_proc(uint32_t count, uint32_t *values,
		uint32_t scl_type)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	uint32_t strategy = SCL_2D_ONLY;
	...
	changed = _iris_scl_perform(true, scl_type, values[1], values[2],
			values[3], values[4], strategy, path_sel, sel_mode);
	if (memc_helper_ops.iris_ioinc_pp_proc)
		changed |= memc_helper_ops.iris_ioinc_pp_proc(true, values[6], values[3], values[4],
			pcfg->frc_setting.disp_hres, pcfg->frc_setting.disp_vres,
			path_sel, sel_mode, &blend_changed);
...
void iris_scl_config(uint32_t count, uint32_t *values)
{
	...
	if (count > max_count || values == NULL) {
		IRIS_LOGE("%s(), invalid parameter!", __func__);
		return;
	}
	...
	if (IRIS_IF_LOGI()) {
		...
		case 5:
			...
			IRIS_LOGI("%s(), proc: 0x%08x, %s: %u, path%u", __func__,
					PROC_INFO(values[1], values[2]),
					values[0] == IRIS_SCL_PP ? "pp pos" : "strategy",
					values[3], path_sel);
			break;
		case 6:
		case 7:
			...
			IRIS_LOGI("%s(), proc: %u, 0x%08x->0x%08x, strategy: %u, path%u",
					__func__, values[0], PROC_INFO(values[1], values[2]),
					PROC_INFO(values[3], values[4]), values[5], path_sel);
		...
		case 9:
			...
			IRIS_LOGI("%s(), proc: 0x%08x->0x%08x, strategy: %u, pos: %u, path%u",
					__func__,
					PROC_INFO(values[1], values[2]),
					PROC_INFO(values[3], values[4]),
					values[5], values[6], path_sel);
			break;
		default:
	...
	_iris_scl_pp_proc(count, values, scl_type);
	_iris_scl_with_pp_proc(count, values, scl_type);
	IRIS_ATRACE_END(__func__);
}
...
static bool _iris_scl_need_reconfig(uint32_t cnn_model, uint8_t *body_ctrl)
{
	if (iris_chip_type == CHIP_IRIS7P) {
		uint32_t CNN_LOWPOWER_MODEL = 0;
	...
	if (iris_chip_type == CHIP_IRIS8) {
		...
		payload = iris_get_ipopt_payload_data(SR_LUT, cnn_model * 0x10 + 0x05, 2);
		if (payload != NULL) {
			uint8_t cnn_s = BITS_GET(payload[0], 8, 0);
			uint8_t cnn_c = BITS_GET(payload[0], 8, 8);
			uint8_t cnn_m = BITS_GET(payload[0], 8, 16);

			IRIS_LOGI("%s(), model param [s, c, m] = [%u, %u, %u]",
				__func__, cnn_s, cnn_c, cnn_m);

			/* The rule is set by DSP. */
			if (cnn_s == 12)
				*body_ctrl = BITS_SET(*body_ctrl, 1, 3, 1);
			*body_ctrl = BITS_SET(*body_ctrl, 3, 0, BITS_MSK(cnn_m));

			/* Without CNN append parameters, set normal as default. */
			if (cnn_s == 0 && cnn_c == 0 && cnn_m == 0)
				*body_ctrl = 0xF;
...
static void _iris_scl_change_model_proc(uint32_t cnn_model)
{
	...
	reconfig_ctrl = _iris_scl_need_reconfig(cnn_model, &body_ctrl);
		...
		uint32_t proc_h, uint32_t proc_v)
{
	...
	IRIS_LOGI("%s(), %s, proc: 0x%08x", __func__,
...
static void _iris_scl_ptsr_sr1d(uint32_t count, uint32_t *values)
{
	bool enable = false;
	uint32_t proc_h = 0;
	uint32_t proc_v = 0;
	...
	proc_h = values[1];
	proc_v = values[2];
	iris_expected_strategy[SCL_DATA_PATH1] = SCL_1D_ONLY;
...
void iris_scl_ptsr_config(uint32_t count, uint32_t *values)
{
	...
	if (IRIS_IF_LOGI()) {
		...
		case 5:
			...
			break;
		case 6:
			IRIS_LOGI("%s(), 1D: %s, %u x %u, strategy: %u", __func__,
					values[0] == 1 ? "on" : "off",
					values[1], values[2], values[3]);
			break;
		case 7:
	...
	_iris_scl_ptsr_sr1d(count, values);
...
void iris_memc_set_pq_level(uint32_t count, uint32_t *values)
{
	...
	case 3:
		...
		break;
	case 4:
		_iris_scl_sr2d_set(SCL_DATA_PATH0, 0, 0, 0, 0);
		_iris_memc_update_strategy(values[1]);
		break;
	case 5:
...
{
	uint8_t level = 0;
	uint8_t ip_lut = SR_IOINC1D_LUT;

	if (count != 1)
		return;

	if (iris_chip_type == CHIP_IRIS7P)
		ip_lut = IOINC1D_PP_LUT;

	/* For detail */
	level = 0x40 + values[0]; /* hs_y */
	iris_init_update_ipopt_t(ip_lut, level, level, 1);
	...
	/* For base */
	level = 0x00 + values[0]; /* base0 and base1 */
	iris_init_update_ipopt_t(IOINC1D_PP_LUT_SHARP, level, level, 0);
...
static void _iris_sr1d_all_filter(uint32_t count, uint32_t *values)
{
	...
	/* For base */
	level = 0x00 + values[4]; /* base0 and base1 */
	iris_init_update_ipopt_t(IOINC1D_PP_LUT_SHARP, level, level, 0);
...
void iris_scl_ioinc_pp_filter(uint32_t count, uint32_t *values)
{
	if (memc_helper_ops.iris_ioinc_pp_filter)
		memc_helper_ops.iris_ioinc_pp_filter(count, values);
}
...
void iris_set_pp_scl_aux(void)
{
	...
	bool blend_changed = false;
	uint32_t ioinc_pos = SCL_PP_CURSOR;
	struct iris_panel_timing_info main_timing, aux_timing;
	...
	if (!pcfg || !pcfg->iris_memc_ops.iris_memc_get_main_panel_timing_info
		|| !pcfg->iris_memc_ops.iris_memc_get_aux_panel_timing_info) {
	...
	if (pcfg->iris_memc_ops.iris_memc_get_aux_panel_timing_info(&aux_timing))
		return;

	if (aux_timing.width == 0 || aux_timing.height == 0)
		return;

	if (iris_bld_scl_pos < SCL_PP_POS_CNT)
		ioinc_pos = iris_bld_scl_pos;

	IRIS_LOGI("%s(), enable pp scl %u for secondary display %ux%u",
			__func__, ioinc_pos,
			aux_timing.width, aux_timing.height);

	if (ioinc_pos == SCL_PP_BLD_OUT) {
		iris_aux_h = aux_timing.width;
		iris_aux_v = aux_timing.height;

		changed = _iris_scl_perform(true, IRIS_SCL_INOUT,
				aux_timing.width, aux_timing.height,
				aux_timing.width, aux_timing.height,
				SCL_2D_ONLY, SCL_DATA_PATH1, PATH_SEL_META);
	}

	if (memc_helper_ops.iris_ioinc_pp_proc)
		changed |= memc_helper_ops.iris_ioinc_pp_proc(true, ioinc_pos,
			aux_timing.width, aux_timing.height,
			main_timing.width, main_timing.height,
			SCL_DATA_PATH0, PATH_SEL_META, &blend_changed);
...
const char *iris_ptsr_status(void)
{
	...
	IRIS_LOGD_IF(status != NULL, "%s(), status: %s", __func__, status);

	return status;
}


#define IRIS_SR_INFO_LEN 40
static unsigned char iris_sr_info[IRIS_SR_INFO_LEN];

static const char *_iris_sr_status(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	uint32_t path_sel = SCL_DATA_PATH1;
	const char *disp_mode = "SINGLE";
	const char *iris_mode = "PT";
	const char *sr_mode = NULL;
	...
	if (!_iris_scl_conf[path_sel].sr_enable)
		return NULL;

	switch (_iris_scl_conf[path_sel].sr_strategy) {
	case SCL_CNN_ONLY:
		sr_mode = "HESR(CNN)";
		break;
	case SCL_CNN_1D:
	...
	memset(iris_sr_info, 0, sizeof(iris_sr_info));
	snprintf(iris_sr_info, IRIS_SR_INFO_LEN, "%s-%s-%s", disp_mode, iris_mode, sr_mode);

	IRIS_LOGI("%s(), SR info: %s", __func__, iris_sr_info);
		...
		size_t count, loff_t *ppos)
{
	...
	size = _iris_get_sr_info(kbuf, size, false);
	if (size >= count)
		size = count - 1;
...
static ssize_t _iris_dbg_write_blend_scl_pos(
...
static ssize_t _iris_dbg_read_blend_scl_pos(struct file *file, char __user *ubuf,
		size_t count, loff_t *ppos)
{
	int size = 0;
	char bp[64];
	...
	size = scnprintf(bp, sizeof(bp), "%d\n", iris_bld_scl_pos);
	if (copy_to_user(ubuf, bp, size))
		return -EFAULT;
	*ppos += size;
...
/* 'echo 1x > d/iris/blend_scl_pos' to enable. */
/* 'echo 0 > d/iris/blend_scl_pos' to reset. */
static const struct file_operations iris_dbg_bld_scl_pos = {
	.open = simple_open,
	.write = _iris_dbg_write_blend_scl_pos,
	.read = _iris_dbg_read_blend_scl_pos,
};
...
int iris_dbgfs_scl_init(void)
{
	...
	if (iris_chip_type == CHIP_UNKNOWN)
		_iris_get_chip_type();

	if (iris_chip_type == CHIP_IRIS7P || iris_chip_type == CHIP_IRIS8)
		if (debugfs_create_file("sr_info", 0644, pcfg->dbg_root, NULL,
					&iris_dbg_sr_info_fops) == NULL)
			IRIS_LOGE("%s(), create file 'sr_info' failed!", __func__);

	if (iris_chip_type == CHIP_IRIS8)
		if (debugfs_create_file("blend_scl_pos", 0644, pcfg->dbg_root, NULL,
					&iris_dbg_bld_scl_pos) == NULL)
			IRIS_LOGE("%s(), create file 'blend_scl_pos' failed!", __func__);

	return 0;
}


static void _srcnn_replace_model(void)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	if (iris_chip_type == CHIP_IRIS8) {
		uint32_t dynamic_model_size = pdata - iris_dynamic_model;

		IRIS_LOGI("%s(), loaded model size: %u, pointer span: %u, %s",
			__func__, iris_loaded_model_size, dynamic_model_size,
			iris_loaded_model_size > dynamic_model_size ? "replace." : "reset to 0.");

		/* '0x94 0x05' for CNN append parameters, use for I8 only. */
		popt = iris_find_ip_opt(SR_LUT, opt_index + 5);
		if (popt != NULL) {
			if (iris_loaded_model_size > dynamic_model_size) {
				/* Replace from dynamic model. */
				pdata += SZ_32;
				for (j = 0; j < popt->cmd_cnt; j++) {
					uint8_t *pbuf = (uint8_t *)popt->cmd[j].msg.tx_buf;
				...
				/* Reset to 0, to make sure it's invalid. */
				for (j = 0; j < popt->cmd_cnt; j++) {
					uint8_t *pbuf = (uint8_t *)popt->cmd[j].msg.tx_buf;

					/* mode addr, 2 * sizeof(uint32_t) */
					pbuf += SZ_8;
					memset((void *)pbuf, 0, popt->cmd[j].msg.tx_len - SZ_8);
				}
			}
		}
	}
...
void iris_memc_helper_setup(void)
{
	...
	if (pcfg->pw_chip_func_ops.iris_memc_helper_setup_)
		pcfg->pw_chip_func_ops.iris_memc_helper_setup_(&memc_helper_ops);
}
