...
static int oplus_usbtemp_dischg_action(struct oplus_mms_wired *chip)
{
	...
	if (get_eng_version() != HIGH_TEMP_AGING) {
		...
		if (oplus_wired_get_typec_mode() != TYPEC_PORT_ROLE_DISABLE) {
			chg_info("typec mode reset to TYPEC_PORT_ROLE_DISABLE");
			oplus_wired_set_typec_mode(TYPEC_PORT_ROLE_DISABLE);
		}
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	} else {
