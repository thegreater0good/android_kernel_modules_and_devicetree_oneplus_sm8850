...
int hybridswap_bind(struct zram *zram, const char *file_name)
{
	...
	backing_dev = bdev_file_open_by_path(file_name, BLK_OPEN_READ | BLK_OPEN_WRITE, zram, NULL);
	...
	nr_pages = (unsigned long)i_size_read(file_inode(backing_dev)) >> PAGE_SHIFT;
