...
static int dbg_msg;
static int pause; /* pause userspace slc service to update slc config */
...
static int cg_priority_prefer = ID_CPU; /*ID_CPU=1, ID_GPU=2*/
static int hint;
...
static unsigned int fr_config[ID_GPU]; /*force ratio, ID_CPU=1, ID_GPU=2, fr_config[0] save cpu, fr_config[1] save gpu ; value range [0 100] */
...
static ssize_t pause_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	char page[32] = {0};
	int ret, val;
...
static ssize_t pause_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	char page[32] = {0};
	int len;
...
static const struct proc_ops pause_proc_ops = {
	.proc_write		= pause_proc_write,
	.proc_read		= pause_proc_read,
	.proc_lseek		= default_llseek,
};
...
static ssize_t force_ratio_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	char page[64] = {0};
	int len;

	len = sprintf(page, "cpu_ratio %d ; gpu_ratio %d ; pause %d\n",
		fr_config[ID_CPU - 1], fr_config[ID_GPU - 1], pause);
	return simple_read_from_buffer(buf, count, ppos, page, len);
}

static ssize_t force_ratio_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	char page[32] = {0};
	int ret, uid, size;
	...
	/* set force from uid and size */
	ret = slbc_force_cache_ratio(uid, size);
	if (!ret) {
		fr_config[uid - 1] = size;
		pr_info("force ratio uid success uid=%d, size=%d\n", uid, size);
	} else {
		fr_config[uid - 1] = 0;
		pr_err("force ratio uid failed, uid=%d, size=%d\n", uid, size);
	}

	/* tracing slc C/G ratio */
	switch (uid - 1) {
	case SERVICE_USAGE_SLC_CPU:
		idc_tp_int(SERVICE_USAGE_SLC_CPU, size);
		break;
	case SERVICE_USAGE_SLC_GPU:
		idc_tp_int(SERVICE_USAGE_SLC_GPU, size);
		break;
	default:
		pr_err("%s: uid not support, uid=%d, size=%d\n", __func__, uid, size);
		break;
	}
...
static const struct proc_ops force_ratio_proc_ops = {
	.proc_write		= force_ratio_proc_write,
	.proc_read		= force_ratio_proc_read,
	.proc_lseek		= default_llseek,
};

static ssize_t cg_priority_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	char page[32] = {0};
	int len;

	len = sprintf(page, "CG priority prefer %d\n", cg_priority_prefer);
	return simple_read_from_buffer(buf, count, ppos, page, len);
}

static ssize_t cg_priority_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	char page[32] = {0};
	int ret, uid;
	...
	if (uid == cg_priority_prefer)
		return count;

	/* set force from uid and size */
	ret = slbc_cg_priority(uid == ID_GPU);
	if (!ret) {
		cg_priority_prefer = uid;
		pr_info("cg priority set %d success\n", uid);
	} else {
		pr_info("cg priority set %d fail\n", uid);
	}

	return count;
}

static const struct proc_ops cg_priority_proc_ops = {
	.proc_write		= cg_priority_proc_write,
	.proc_read		= cg_priority_proc_read,
	.proc_lseek		= default_llseek,
};
...
static ssize_t dis_cdwb_proc_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	cdwb_disable = !!input;
#if (IS_ENABLED(CONFIG_MTK_SLBC_MT6991) || IS_ENABLED(CONFIG_MTK_SLBC_MT6993))
	if (ipi_ops_ref)
		ipi_ops_ref->slbc_dcc_ctrl(cdwb_disable);
	else
		pr_err("ipi_ops_ref is NULL, slbc_dcc_ctrl failed!!!\n");
#elif IS_ENABLED(CONFIG_MTK_SLBC_MT6989)
...
void get_cg_force_size(unsigned int *cpu,  unsigned int *gpu)
{
	*cpu = f_config[0];
	*gpu = f_config[1];
}
EXPORT_SYMBOL(get_cg_force_size);

void get_cache_usage_size(int *cpu,  int *gpu,  int *other)
{
	slbc_get_cache_usage(cpu, gpu, other);
}
EXPORT_SYMBOL(get_cache_usage_size);

int get_oplus_slc_dis_cdwb_status(void)
{
	return cdwb_disable;
}
EXPORT_SYMBOL(get_oplus_slc_dis_cdwb_status);

void get_cg_force_ratio(unsigned int *cpu, unsigned int *gpu)
{
	*cpu = fr_config[0];
	*gpu = fr_config[1];
}
EXPORT_SYMBOL(get_cg_force_ratio);

int get_cache_priority_status(void)
{
	return cg_priority_prefer;
}
EXPORT_SYMBOL(get_cache_priority_status);
...
static void create_slc_proc(void)
{
	...
	/* slbc uid force ratio set */
	proc_create_data("force_ratio", 0664, oplus_slc_dir, &force_ratio_proc_ops, NULL);
	/* slbc cg priority set */
	proc_create_data("priority", 0664, oplus_slc_dir, &cg_priority_proc_ops, NULL);
	/* slbc enable */
	...
	proc_create_data("pause", 0664, oplus_slc_dir, &pause_proc_ops, NULL);
}

static int oplus_slc_pm_notify(struct notifier_block *nb, unsigned long mode, void *_unused)
{
	switch (mode) {
...
static struct notifier_block oplus_slc_pm = {
	.notifier_call = oplus_slc_pm_notify,
};
...
static int __init oplus_slc_init(void)
{
	...
	ret = register_pm_notifier(&oplus_slc_pm);
	if (ret)
		pr_err("oplus_slc failed to register PM notifier.\n");

	oplus_slc_indicator_init(oplus_slc_dir);
...
static void __exit oplus_slc_exit(void)
{
	...
	osml_exit();
	unregister_pm_notifier(&oplus_slc_pm);
