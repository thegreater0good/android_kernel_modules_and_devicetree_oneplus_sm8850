...
static char *monitor_case[MONITOR_SIZE] = {
	...
	"mem_bw", "mem_calc_bw", "LLCC_freq", "venc_count", "LLCC_bw",
...
static char pevent_name[MAX_PEVENT_SIZE][MAX_STRING_LEN] = {
	"raw-inst-retired", "raw-cpu-cycles", "raw-l3d-cache", "raw-l3d-cache-refill",
	"raw-stall", "raw-stall-backend-mem", "mem_access",
};
...
static int ioc_init(void)
{
	...
	driver_class = class_create(OSML_CTL_NODE);
...
static void collect_info(void)
{
	...
	list_for_each_entry(event, &list_event_head[OSML_EVENT], osml_event_node) {
		...
		case EMI_OPY_BW:
		{
			...
			if ((emi_opy_bw[EMI_OPY_BW_CPU] + emi_opy_bw[EMI_OPY_BW_MM]) >= emi_opy_bw[EMI_OPY_BW_TOTAL])
				emi_opy_bw[EMI_OPY_BW_GPU] = 0;
			else
				emi_opy_bw[EMI_OPY_BW_GPU] = (emi_opy_bw[EMI_OPY_BW_TOTAL] - emi_opy_bw[EMI_OPY_BW_CPU] - emi_opy_bw[EMI_OPY_BW_MM]);
		...
		case EMI_TP:
		{
			...
			if ((emi_tp[EMI_TP_CPU] + emi_tp[EMI_TP_MM]) >= emi_tp[EMI_TP_TOTAL])
				emi_tp[EMI_TP_GPU] = 0;
			else
				emi_tp[EMI_TP_GPU] = (emi_tp[EMI_TP_TOTAL] - emi_tp[EMI_TP_CPU] - emi_tp[EMI_TP_MM]);
		...
		case LLCC_FREQ:
			...
			break;
		case VENC:
			pmonitor.buf[idx] = slbc_sram_read(SLBC_DCC_COUNT);;
			break;
		case LLCC_BW:
			pmonitor.buf[idx] = -1;
			break;
...
{
	unsigned int cpu, clus_idx = 0, i, total_state = 0;
