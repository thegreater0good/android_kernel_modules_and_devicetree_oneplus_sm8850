...
static void attach_task(struct rq *rq, struct task_struct *p)
{
	...
	/*check_preempt_curr changed to wakeup_preempt by new feature CONFIG_SCHED_PROXY_EXEC KERNEL 6.12*/
			...
			int src_cpu, int dst_cpu, u64 *time)
{
	...
	for (node = rb_first_cached(&orq->ux_list); node; node = rb_next(node)) {
		...
		if (!task_is_runnable_on_runqueue(task))
			continue;
...
static int oplus_get_cluster_util(int cpu)
{
	...
	for_each_cpu(cpu, &search_cpus) {
		...
		total_capacity += oplus_capacity_orig_of(cpu);
	}
