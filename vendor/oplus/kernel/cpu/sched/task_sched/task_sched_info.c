...
static unsigned long  get_wchan_new(struct task_struct *p)
{
	...
	nr_entries = stack_trace_save_tsk(p, stacks, ARRAY_SIZE(stacks), 0);
	if (nr_entries)
		return stacks[0];
	return 0;
}
...
static void update_backtrace_info(struct task_struct *p, u64 start_time, int type, u64 delay)
{
	...
	task_backtrace.sched_info_two = (u64)get_wchan_new(p);
...
u64 update_limit_call_info(bool flag_worker, int target_tid, char *comm)
{
	...
	if (flag_worker) {
		...
		freq_worker = kzalloc(sizeof(*freq_worker), GFP_ATOMIC);
		if (IS_ERR_OR_NULL(freq_worker)) {
			sched_err("kzalloc freq_worker failed\n");
			return 0;
		}
		...
		freq_thread = kzalloc(sizeof(*freq_thread), GFP_ATOMIC);
		if (IS_ERR_OR_NULL(freq_thread)) {
			sched_err("kzalloc freq_thread failed\n");
			return 0;
		}
...
static int record_limit_tid_min(struct notifier_block *notifier, unsigned long freq, void *data)
{
	...
	if (target_cpu_freq_limit) {
		...
		if (current->flags & PF_WQ_WORKER) {
			strscpy_pad(target_cpu_freq_limit->min_comm, current->comm, sizeof(target_cpu_freq_limit->min_comm));
...
static int record_limit_tid_max(struct notifier_block *notifier, unsigned long freq, void *data)
{
	...
	if (target_cpu_freq_limit) {
		...
		if (current->flags & PF_WQ_WORKER) {
			strscpy_pad(target_cpu_freq_limit->max_comm, current->comm, sizeof(target_cpu_freq_limit->max_comm));
...
static void sched_switch_handler(void *data, bool preempt, struct task_struct *prev, struct task_struct *next, unsigned int prev_state)
{
	if (!task_sched_info_enable)
		return;
