...
static char render_names[RENDER_COUNT][MAX_RENDER_NAME_LEN] = { "UnityMain", "UnityGfx" };
...
static int namencmp(const char* comm, const char* name) {
	int len = strlen(name);
	return strncmp(comm, name, len > MAX_RENDER_NAME_LEN ? MAX_RENDER_NAME_LEN : len);
}

static noinline bool is_render_thread(struct task_struct *p)
{
	return ((namencmp(p->comm, render_names[0])) == 0 ||
	       (namencmp(p->comm, render_names[1])) == 0);
}

static noinline void update_renders(struct task_struct *p)
{
	if (namencmp(p->comm, render_names[0]) == 0) {
		renders[0] =
...
void htb_notify_render_names_changed(const char* name0, const char* name1)
{
	unsigned long flags;

	if (strlen(name0) > MAX_RENDER_NAME_LEN ||
		strlen(name1) > MAX_RENDER_NAME_LEN) {
		return;
	}

	raw_spin_lock_irqsave(&htb_spinlock, flags);
	strncpy(render_names[0], name0, strlen(name0));
	strncpy(render_names[1], name1, strlen(name1));
	raw_spin_unlock_irqrestore(&htb_spinlock, flags);
}
