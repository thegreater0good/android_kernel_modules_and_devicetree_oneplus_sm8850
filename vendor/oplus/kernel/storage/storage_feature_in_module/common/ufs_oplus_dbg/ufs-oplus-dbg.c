...
void ufs_compl_cmd_handle(void *data, struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (lrbp && lrbp->cmd) {
		...
		if ((cmd->cmnd[0] == READ_10 || cmd->cmnd[0] == WRITE_10
				|| cmd->cmnd[0] == READ_16 || cmd->cmnd[0] == WRITE_16)
...
void ufs_iostack_init(struct delayed_work *dwork)
{
	INIT_DELAYED_WORK(dwork, iostack_monitor_work);
	schedule_delayed_work(dwork, IOSTACK_WORK_DELAY);
}
EXPORT_SYMBOL_GPL(ufs_iostack_init);
