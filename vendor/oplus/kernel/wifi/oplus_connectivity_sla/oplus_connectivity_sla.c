...
enum {
	...
	OPLUS_SLA_CMD_SET_DOWNLOAD_LIST_APP       = 21,
	OPLUS_SLA_CMD_SET_DOWNLOAD_WEIGHT         = 22,
	__SLA_MSG_MAX,
};
...
struct dual_sta_weight {
	int main_weight;
	int second_weight;
};

static struct dual_sta_weight download_weight = {
	.main_weight = 100,
	.second_weight = 0
};
...
static u32 main_wlan_count = 0;
static u32 second_wlan_count = 0;
				...
				struct sk_buff *skb)
{
	...
	if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
		...
		u32 total = main_wlan_count + second_wlan_count;
		u32 main_weight = download_weight.main_weight;
		u32 second_weight = download_weight.second_weight;

		if (total == 0 ||
			(main_wlan_count * second_weight < second_wlan_count * main_weight)) {
			choose_mark = MAIN_WLAN_MARK;
			main_wlan_count++;
		} else {
			choose_mark = SECOND_WLAN_MARK;
		...
		if (NULL != sk) {
			...
			if (NULL != tp && NULL != th && th->ack && th->syn) {
                ...
                */
				...
				if (tcp_opt.saw_tstamp && tcp_opt.rcv_tsecr &&
					!between(tcp_opt.rcv_tsecr, tp->retrans_stamp, tcp_time_stamp_ts(tp))) {
#else
				if (tcp_opt.saw_tstamp && tcp_opt.rcv_tsecr &&
...
static int oplus_sla_set_download_app_uid(struct nlattr *nla)
{
	u32 *info = (u32 *)NLA_DATA(nla);
	memset(&download_app_list, 0x0, sizeof(struct oplus_dual_sta_info));
	download_app_list.count = info[0];

	if (download_app_list.count > 0
		&& download_app_list.count < DUAL_STA_APP_NUM) {
		int i;

		for (i = 0; i < download_app_list.count; i++) {
			download_app_list.uid[i] = info[i + 1];
			debug("set_download_app_uid count=%d, uid[%d]=%d\n",
...
static int oplus_dualsta_update_download_weight(struct nlattr *nla)
{
	int *weight = (int *)NLA_DATA(nla);
	sla_write_lock();

	download_weight.main_weight = weight[0];
	download_weight.second_weight = weight[1];

	main_wlan_count = 0;
	second_wlan_count = 0;

	debug("download weight: main:%u second:%u\n",
		download_weight.main_weight, download_weight.second_weight);
	sla_write_unlock();
	return 0;
}
	...
	struct genl_info *info)
{
	...
	case OPLUS_SLA_CMD_SET_DOWNLOAD_LIST_APP:
		ret = oplus_sla_set_download_app_uid(nla);
		break;

	case OPLUS_SLA_CMD_SET_DOWNLOAD_WEIGHT:
		ret = oplus_dualsta_update_download_weight(nla);
		break;
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0))
	oplus_sla_table_hrd = register_net_sysctl_sz(&init_net, "net/oplus_sla",
			oplus_sla_sysctl_table, ARRAY_SIZE(oplus_sla_sysctl_table));
#else
