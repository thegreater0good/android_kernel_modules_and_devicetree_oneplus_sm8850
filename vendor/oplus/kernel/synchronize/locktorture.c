...
char *kbuf;
#define CONFIG_SHOW_MAX_BUF   (2 * 1024)
...
static void acquire_start(void)
{
	...
	if (unlikely(IS_ERR_OR_NULL(ots))) {
		pr_err("locktorture : ots == NULL !!!!!!!\n");
		return;
	}

	/* start must always pair with finish. */
	if (ots->lkinfo.acquire_stamp)
		pr_err("locktorture : why torture stamp not 0 !!!!!!!\n");
...
static void acquire_finish(int lock_type, int rw, enum thread_attr grp)
{
	...
	if (0 == ots->lkinfo.acquire_stamp)
		pr_err("locktorture : why torture stamp is 0 !!!!!!!\n");
...
static int torture_configs_show(struct seq_file *m, void *v)
{
	...
	kbuf = (char *)kmalloc(CONFIG_SHOW_MAX_BUF, GFP_KERNEL);
	if (!kbuf)
		return -ENOMEM;
...
static void print_thread_attr(void)
{
	...
	if (current->prio < MAX_RT_PRIO) {
		...
		pr_err("locktorture : ---Create RT thread, num : %d---\n",
	...
	if (!IS_ERR_OR_NULL(ots) && (ots->ux_state & SCHED_ASSIST_UX_MASK)) {
		...
		pr_err("locktorture : ---Create UX thread, num : %d---\n",
...
static int lock_torture_writer(void *arg)
{
	...
	if (set_thread_attr(lwsp->attr, 1) < 0)
		pr_err("locktorture : Failed to call set_thread_attr \n");
	msleep(1);
	print_thread_attr();
	do {
		acquire_start();
	...
	pr_err("locktorture : ---Exit %s thread, num : %d---\n",
...
static int lock_torture_start(void)
{
    ...
    if (cxt.cfg.cur_ops->readlock) {
        ...
        if (reader_tasks == NULL) {
            pr_err("locktorture : reader_tasks: Out of memory");
            kfree(writer_tasks);
            writer_tasks = NULL;
            firsterr = -ENOMEM;
            goto unwind;
