...
static int oplus_qpnp_pon_wd_config(bool enable)
{
	...
	if (is_pon_gen3(sys_reset_dev)) {
		if (pmicwd->wd_base == QPNP_PON_GEN3_WD_BASE) {
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_WD_ENABLE(pmicwd), enable ? QPNP_PON_WD_EN : 0);
			if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
			    QPNP_PON_GEN3_WD_ENABLE(pmicwd), rc);
			}
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_WD_PET(pmicwd), enable ? QPNP_PON_WD_EN : 0);
			if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
			    QPNP_PON_GEN3_WD_PET(pmicwd), rc);
			}
			return 0;
		}
		return dup_qpnp_pon_masked_write(sys_reset_dev,
							QPNP_PON_GEN3_WD_RST_S2_CTL2(pmicwd),
							QPNP_PON_WD_EN, enable ? QPNP_PON_WD_EN : 0);
	} else {
		return dup_qpnp_pon_masked_write(sys_reset_dev,
							QPNP_PON_WD_RST_S2_CTL2(pmicwd),
							QPNP_PON_WD_EN, enable ? QPNP_PON_WD_EN : 0);
	}
}
...
static int oplus_pon_pbs_int_cfg(void) {
	...
	if (is_pon_gen3(sys_reset_dev)) {
		if (pmicwd->wd_base == QPNP_PON_GEN3_WD_BASE) {
/*
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_INT_SET_TYPE(pmicwd), 0x02);
		    if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_SET_TYPE(pmicwd), rc);
			}
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_INT_POLARITY_HIGH(pmicwd), 0x02);
		    if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_POLARITY_HIGH(pmicwd), rc);
		    }
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_INT_POLARITY_LOW(pmicwd), 0x00);
		    if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_POLARITY_LOW(pmicwd), rc);
		    }
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_INT_LATCHED_CLR(pmicwd), 0x00);
		    if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_LATCHED_CLR(pmicwd), rc);
		    }
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_INT_EN_SET(pmicwd), 0x02);
		    if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_EN_SET(pmicwd), rc);
		    }
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_INT_EN_CLR(pmicwd), 0x02);
		    if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_EN_CLR(pmicwd), rc);
		    }
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_WD_INT_MID_SEL(pmicwd), 0x03);
		    if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_INT_MID_SEL(pmicwd), rc);
		    }
*/
			return rc;
		}
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_INT_SET_TYPE(pmicwd),
		...
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_INT_POLARITY_HIGH(pmicwd),
		...
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_INT_POLARITY_LOW(pmicwd),
				PMIC_WD_INT_BIT_MASK, 0x00);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_POLARITY_LOW(pmicwd), rc);
		...
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_INT_LATCHED_CLR(pmicwd),
				PMIC_WD_INT_BIT_MASK, PMIC_WD_INT_BIT_MASK);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_LATCHED_CLR(pmicwd), rc);
		...
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_INT_EN_SET(pmicwd),
				PMIC_WD_INT_BIT_MASK, PMIC_WD_INT_BIT_MASK);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_EN_SET(pmicwd), rc);
		...
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_INT_EN_CLR(pmicwd),
				PMIC_WD_INT_BIT_MASK, PMIC_WD_INT_BIT_MASK);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_INT_EN_CLR(pmicwd), rc);
...
static int qpnp_pon_wd_timer(unsigned char timer, enum pon_power_off_type reset_type) {
	...
	if (is_pon_gen3(sys_reset_dev)) {
		if (pmicwd->wd_base == QPNP_PON_GEN3_WD_BASE) {
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_WD_BITE_TIME_LSB(pmicwd), 0x06);
			if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_BITE_TIME_LSB(pmicwd), rc);
			}
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_WD_BITE_TIME_MSB(pmicwd), 0x7c);
			if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_BITE_TIME_MSB(pmicwd), rc);
			}
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_WD_RESET_TYPE(pmicwd), reset_type);
			if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_RESET_TYPE(pmicwd), rc);
			}
			return rc;
		 }
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_WD_RST_S2_TIMER(pmicwd),
				QPNP_PON_WD_S2_TIMER_MASK, s2_timer);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_RST_S2_TIMER(pmicwd), rc);
		}

		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_WD_RST_S1_TIMER(pmicwd),
				QPNP_PON_WD_S1_TIMER_MASK, s1_timer);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_RST_S1_TIMER(pmicwd), rc);
		}

		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_GEN3_WD_RST_S2_CTL(pmicwd),
				QPNP_PON_S2_CNTL_TYPE_MASK, reset_type);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_RST_S2_CTL(pmicwd), rc);
		}
	} else {
		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_WD_RST_S2_TIMER(pmicwd),
				QPNP_PON_WD_S2_TIMER_MASK, s2_timer);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_WD_RST_S2_TIMER(pmicwd), rc);
		}

		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_WD_RST_S1_TIMER(pmicwd),
				QPNP_PON_WD_S1_TIMER_MASK, s1_timer);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_WD_RST_S1_TIMER(pmicwd), rc);
		}

		rc = dup_qpnp_pon_masked_write(pmicwd->pon, QPNP_PON_WD_RST_S2_CTL(pmicwd),
				QPNP_PON_S2_CNTL_TYPE_MASK, reset_type);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_WD_RST_S2_CTL(pmicwd), rc);
		}
	}
...
int qpnp_pon_wd_pet(struct qpnp_pon *pon) {
	...
	if (is_pon_gen3(sys_reset_dev)) {
		if (pmicwd->wd_base == QPNP_PON_GEN3_WD_BASE) {
			rc = dup_qpnp_pon_write(pmicwd->pon, QPNP_PON_GEN3_WD_PET(pmicwd), 0x80);
			if (rc) {
			    PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_PET(pmicwd), rc);
			}
		...
		rc = dup_qpnp_pon_masked_write(pon, QPNP_PON_GEN3_WD_RESET_PET(pmicwd),
				QPNP_PON_WD_RESET_PET_MASK, 1);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_GEN3_WD_RESET_PET(pmicwd), rc);
		}
	} else {
		rc = dup_qpnp_pon_masked_write(pon, QPNP_PON_WD_RESET_PET(pmicwd),
				QPNP_PON_WD_RESET_PET_MASK, 1);
		if (rc) {
			PWD_ERR("Unable to write to addr=%x, rc(%d)\n",
					QPNP_PON_WD_RESET_PET(pmicwd), rc);
		}
	}
...
static ssize_t pmicwd_config_proc_read(struct file *file, char __user *buf,
	...
	if (is_pon_gen3(sys_reset_dev)) {
		if (pmicwd->wd_base == QPNP_PON_GEN3_WD_BASE) {
			regmap_read(pmicwd->pon->regmap, QPNP_PON_GEN3_WD_ENABLE(pmicwd), &val);
		}
		regmap_read(pmicwd->pon->regmap, QPNP_PON_GEN3_WD_RST_S2_CTL2(pmicwd), &val);
	} else {
		regmap_read(pmicwd->pon->regmap, QPNP_PON_WD_RST_S2_CTL2(pmicwd), &val);
	}
...
void pmicwd_init(struct platform_device *pdev) {
	...
	regmap_read(pmicwd->pon->regmap, QPNP_PON_RT_STS(pmicwd), &pon_rt_sts);
...
static void pmicwd_remove(struct platform_device *pdev)
{
	if (pmicwd) {
		if(pmicwd->pmicwd_state) {
