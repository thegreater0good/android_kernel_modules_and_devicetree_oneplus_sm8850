...
/**********************************************
PONSTS[0XC]
6		STS_RTC_INT		Power on for RTC
5		STS_ENON		Power on for EN
4		STS_RBOOT		Power on for cold reset
3		STS_SPAR		Power on for SPAR
2		STS_CHRIN		Power on for charger insertion
1		STS_RTCA		Power on for RTCA
0		STS_PWRKEY		Power on for PWREKY press
**********************************************/
static char * const mt6661_S4_pon_reason_str[] = {
...
/**********************************************
[PMIC]POFFSTS[0xD]
15		STS_UVLO_VBB   Power off for VBB UVLO event
14		STS_OVLO	   Power off for OVLO event
13		STS_PKSP	   PWRKEY short press event
...
0		STS_UVLO	   Power off for VSYS UVLO event
**********************************************/
static char * const mt6661_S4_pon_poff_reason_str[] = {
	[0]=":Power off for VSYS UVLO event",
	...
	[15]=":Power off for VBB UVLO event",
};

/**********************************************
[PMIC]POFFSTS2[0xF]
15		STS_UDS_EXP	           Power off for exception in UDS mode
14		STS_EXIT_UDS	       UDS exit happend
13		STS_FALUTB_RX	       Power off for FALUTB_RX
12		STS_OVLO_VBST_DELTA	   Power off for VBST_DELTA OVLO
11		STS_OVLO_VBST_ABS	   Power off for VBST_ABS OVLO
10		STS_OVLO_VBB_DLDO 	   Power off for VBB_DLDO OVLO
9		STS_UVLO_VBB_DLDO	   Power off for VBB_DLDO UVLO
8		STS_UVLO_VBST	       Power off for VBST UVLO
7		STS_BLE_EXP	           Power off for exception in BLE mode
6		STS_EXIT_BLE	       BLE exit happened
4		STS_OVLO_VBB	       Power off for VBB OVLO
3		STS_PMICWDT_TMR2	   Power off for PMIC self WDT by Timer2
2		STS_PMICWDT_TMR1	   Power off for PMIC self WDT by Timer1
1		STS_EN_EXOFF           Power off for EN exception off
0		STS_EN_SQOFF           Power off for EN sequence off
**********************************************/
static char * const mt6661_S4_pon_poff_reason2_str[] = {
	[0]=":Power off for EN sequence off",
	[1]=":Power off for EN exception off",
	[2]=":Power off for PMIC self WDT by Timer1",
	[3]=":Power off for PMIC self WDT by Timer2",
	[4]=":Power off for VBB OVLO",
	[5]=":Reserved bit, meaningless",
	[6]=":BLE exit happened",
	[7]=":Power off for exception in BLE mode",
	[8]=":Power off for VBST UVLO",
	[9]=":Power off for VBB_DLDO UVLO",
	[10]=":Power off for VBB_DLDO OVLO",
	[11]=":Power off for VBST_ABS OVLO",
	[12]=":Power off for VBST_DELTA OVLO",
	[13]=":Power off for FALUTB_RX",
	[14]=":UDS exit happend",
	[15]=":Power off for exception in UDS mode",
};

/**********************************************
[PMIC]POFFSTS4_5[0x11]
7		STS_THR_LOC         Power off thermal location
6		STS_THR_LOC         Power off thermal location
5		STS_THR_LOC         Power off thermal location
4		STS_THR_LOC         Power off thermal location
2		STS_FAULTB_CONN_RX  Power off for FAULTB_CONN_RX
1		STS_OV              Power off for BUCK OV
0		STS_SC              Power off for BUCK SC
**********************************************/
static char * const mt6661_S4_pon_poff_reason4_5_str[] = {
	[0]=":Power off for BUCK SC",
	[1]=":Power off for BUCK OV",
	[2]=":Power off for FAULTB_CONN_RX",
	[3]=":Reserved bit, meaningless",
	[4]=":Power off thermal location",
	[5]=":Power off thermal location",
	[6]=":Power off thermal location",
	[7]=":Power off thermal location",
};
void mt6661_S4_pmic_show(struct PMICHistoryKernelStruct *pmic_history_ptr, char page[], int *len, u64 pmic_history_count){

	u8 pmic_device_index = 0;
	struct PMICRecordKernelStruct pmic_first_record = {0};
	struct PMICRegStruct pmic_reg_value = {0};

	int pon_reason_bit = -1;
	int poff_reason_bit = -1;
	int poff_reason2_bit = -1;
	int poff_reason4_5_bit = -1;

	unsigned int cur_pon_reason = 0;
	unsigned int cur_poff_reason = 0;
	unsigned int cur_poff_reason2 = 0;
	unsigned int cur_poff_reason4_5 = 0;
	unsigned int cur_pon_reason_bak = 0;
	unsigned int cur_poff_reason_bak = 0;
	unsigned int cur_poff_reason2_bak = 0;
	unsigned int cur_poff_reason4_5_bak = 0;
	unsigned int cur_pg_sdn_sts0 = 0;
	unsigned int cur_pg_sdn_sts1 = 0;
	unsigned int cur_oc_sdn_sts0 = 0;
	unsigned int cur_oc_sdn_sts1 = 0;
	...
	for (;pmic_device_index<pmic_history_count;pmic_device_index++) {
		...
		if (pon_reason_bit>=0 && pon_reason_bit<=6) {
			printk(KERN_INFO "mt6661_S4_show_pon_reason = %d\n",pon_reason_bit);
			(*len) += snprintf(&page[(*len)],2048-(*len), "PMIC->pon_reason|%u|0x%02X|:%s\n",
						pmic_device_index,
						cur_pon_reason,
						mt6661_S4_pon_reason_str[pon_reason_bit]);
		} else {
			printk(KERN_INFO "mt6661_S4_show_pon_reason = %d\n",pon_reason_bit);
			(*len) += snprintf(&page[(*len)],2048-(*len), "PMIC->pon_reason|%u|0x%02X|:%s\n",
						pmic_device_index,
		...
		cur_poff_reason4_5 = pmic_reg_value.poff_reason4_5;
		...
		cur_poff_reason4_5_bak = cur_poff_reason4_5;
		while (0 != cur_poff_reason4_5_bak) {
			cur_poff_reason4_5_bak = cur_poff_reason4_5_bak >> 1;
			poff_reason4_5_bit++;
		}
		...
		if(poff_reason2_bit>=0 && poff_reason2_bit<=15){
			(*len) += snprintf(&page[(*len)], 2048-(*len), "PMIC->poff_reason2|%u|0x%x|:%s\n",
				pmic_device_index,
				cur_poff_reason2,
				mt6661_S4_pon_poff_reason2_str[poff_reason2_bit]);
		}else{
			(*len) += snprintf(&page[(*len)], 2048-(*len), "PMIC->poff_reason2|%u|0x%x|:%s\n",
				pmic_device_index,
				cur_poff_reason2,
		...
		if(poff_reason4_5_bit >= 0 && poff_reason4_5_bit <= 7){
			(*len) += snprintf(&page[(*len)], 2048-(*len), "PMIC->poff_reason4_5|%u|0x%x|:%s\n",
				pmic_device_index,
				cur_poff_reason4_5,
				mt6661_S4_pon_poff_reason4_5_str[poff_reason4_5_bit]);
		}else{
			(*len) += snprintf(&page[(*len)], 2048-(*len), "PMIC->poff_reason4_5|%u|0x%x|:%s\n",
				pmic_device_index,
				cur_poff_reason4_5,
...
static ssize_t pmic_monitor_show(struct kobject *kobj,
	...
	} else if (pmic_history_ptr->pmic_record[0].pmic_pon_poff_reason[0].chip_code == 97) {
		printk(KERN_INFO "mt6661_S4_pmic_show\n");
		mt6661_S4_pmic_show(pmic_history_ptr, page, &len, pmic_history_count);
	} else {
