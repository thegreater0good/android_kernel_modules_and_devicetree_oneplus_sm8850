...
static int init_pmic_history_fdt(void)
{
	...
	for(i = 0; i < log_count; i++) {
		...
		ret = of_property_read_u32(np,"poff_reason4_5",&reg->poff_reason4_5);
		if (ret && (ret != -EINVAL)) {
			pr_err("%s don't find poff_reason4_5\n", __func__);
			return ret;
		}
		...
		ret = of_property_read_u32(np,"oplus_porstb_uvlo_flag",&reg->oplus_porstb_uvlo_flag);
                if (ret && (ret != -EINVAL)) {
                        pr_err("%s don't find oplus_uvlo_flag\n", __func__);
                        return ret;
                }
